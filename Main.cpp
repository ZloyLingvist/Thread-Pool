...
void help(){
	...
	cout << "-q -- quescap <int> -- объЄм очереди задач " << endl;
...
int main(int argc, char* argv[]){
	...
	if (workers == 0){
		workers = std::thread::hardware_concurrency() + 1; //запускаем на максимально возможном дл€ системы количестве потоков
	}
	...
	for (int i = 0; i < quescap; i++){
		...
		thread_pool.push(1, example_function1, value,"–аботаю с числами в цикле\n");
		thread_pool.push(2, example_function2, value,"ѕишу в текстовый файл\n");
		thread_pool.push(3, example_function3, value,"”множаю матрицы\n");
	}
