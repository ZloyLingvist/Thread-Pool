...
void Testing_class::create_file(int p, vector<string> &filenames) {
	...
	if (p != 0) {
		...
		for (i = 0; i < file_amount; i++) {
			...
			str1 = str1 + ".txt";
			filenames.push_back("files\\"+str1);
			out.open("files\\"+str1);
	...
	for (size_t i = 1; i < test_vector.size() - minus; i++) {
		if (test_vector[i].length() > 0) {
			...
			if (p == 2 || test_vector.back() == "2"){
					string res = test_vector[i];
					res = res.insert(6, "temp_");//если папка не files\\ то изменить !!!!
					o_ar.compress(test_vector[i].c_str(), res.c_str());
					myar.add_to_archive(res.c_str());
			}

			if (p == 3 || test_vector.back() == "3") {
					string res = test_vector[i];
					res = res.insert(6, "temp_");//если папка не files\\ то изменить !!!!
					l_ar.compress(test_vector[i].c_str(), res.c_str());
					myar.add_to_archive(res.c_str());
			}
		}
	}
...
bool Testing_class::extraction_function(int p,int log) {
	...
	for (size_t i = 1; i < test_vector.size() - minus; i++) {
		if (test_vector[i].length() > 0) {
			...
			if (p == 2 || test_vector.back() == "2") {
					string res = test_vector[i];
					res = res.insert(6, "res_");//если папка не files\\ то изменить !!!!

					string res2 = test_vector[i];
					res2 = res2.insert(6, "temp_");//если папка не files\\ то изменить !!!!

					o_ar.decompress(res2.c_str(), res.c_str());

					if (isFilesEqual(test_vector[i].c_str(), res.c_str()) != true) {
						return false;
					};

					size_after = size_after + filesize(res.c_str());
			}
			...
			if (p == 3 || test_vector.back() == "3") {
				...
				l_ar.decompress(res2.c_str(), res.c_str());
...
void Testing_class::cleaner() {
	for (size_t i = 1; i < test_vector.size(); i++) {
		if (test_vector[i] == "files\\in1.txt" || test_vector[i] == "files\\in2.txt") {
			continue;
		}
...
void my::test::function_block(TaskQueue &queue,int log) {
	...
	string t_n1 = "Add to archive ";
	string t_n2 = "Extract from archive ";
	string t_n3 = "Decompress ";
	string t_n4 = "Compare ";
	string t_n5 = "Clean and write iteration";
	int k = 10;

	for (int i = 0; i < queue.capacity_(); i++) {
		...
		t7.add_task(t_n3, 3+k, decompress_lib, "files\\newarch2.tar", "files\\tempfile1_lib.txt", "files\\tempfile2_lib.txt", "files\\res1_lib.txt", "files\\res2_lib.txt");
		...
		queue.push(t4);
		queue.push(t8);
		k = k +10;//чтобы не было пересечений между приоритетами
	}
}
